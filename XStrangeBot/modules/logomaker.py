...
from XStrangeBot import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        fpath_ = glob.glob("./XStrangeBot/resources/fonts/*")
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Emiko Robot](https://t.me/XStrangeBot)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("./XStrangeBot/resources/fonts"):
            os.remove(font_)
